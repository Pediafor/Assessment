...
class RabbitMQConnection {
  ...
  private async setupInfrastructure(): Promise<void> {
    ...
    // Submission Events Exchange (Topic Exchange for flexibility)
    await this.assertExchange('submission.events', 'topic');
    
    // Dead Letter Exchange for failed messages
    await this.assertExchange('dead.letter', 'direct');

    // Setup queues for submission events
    await this.assertQueue('submission.submitted', {
      durable: true,
      arguments: {
    ...
    await this.assertQueue('submission.updated', {
      durable: true,
      arguments: {
        'x-dead-letter-exchange': 'dead.letter',
        'x-dead-letter-routing-key': 'failed.submission.updated'
      }
    });

    await this.assertQueue('submission.graded', {
      durable: true,
      arguments: {
        'x-dead-letter-exchange': 'dead.letter',
        'x-dead-letter-routing-key': 'failed.submission.graded'
      }
    });

    // Bind queues to exchanges
    await this.bindQueue('submission.submitted', 'submission.events', 'submission.submitted');
    await this.bindQueue('submission.updated', 'submission.events', 'submission.updated');
    await this.bindQueue('submission.graded', 'submission.events', 'submission.graded');
